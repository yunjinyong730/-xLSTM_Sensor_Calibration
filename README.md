...
> **한 줄 요약**  
> TimeXer는 내생(예측 대상) 시계열은 **패치 단위(Self-Attention)** 로, 외생(보조) 시계열은 **변수 단위(Cross-Attention)** 로 처리하며, **글로벌 토큰**을 매개로 두 정보를 정교하게 결합해 다양한 벤치마크에서 SOTA 성능을 보이는 간단·일반 목적의 트랜스포머 설계입니다.
